...
Ajax.Request.prototype = Object.extend(new Ajax.Base(),
{
	initialize: function( url, options )
	{
		...
		if ( !url ) url = $A((""+document.location).split("/")).last();
...
/*		for ( var i = 0; i < requestHeaders.length; i += 2 )
		...
		for ( var i = 0; i < requestHeaders.length; i += 2 )
		{
			_oud = _headers[requestHeaders[i]] ? _headers[requestHeaders[i]] + ";" : '';
			_headers[requestHeaders[i]] = _oud + requestHeaders[i+1];
		}

		for ( _name in _headers )
		{
			_value = _headers[_name];
			this.transport.setRequestHeader( _name, _value );
		}
	},
	...
	header: function( name )
	{
		...
		{
//			alert("foutje in header ophalen!?");
		}
	},
