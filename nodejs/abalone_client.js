...
Abalone.prototype = {
	 ...
	 * Constructor
	 */
	construct: function(container, playerColor) {
		var self = this;

		this.$container = $(container);

		this.m_szPlayerColor = playerColor;
...
_log('Attaching DOM listeners for "' + match + '"');
	...
	/**
	 * Populate HTML board with Nodejs balls
	 */
	populateMap: function(balls) {
		// Unset
		this.$container.find('.ball').removeClass('black').removeClass('white').each(function(i, b) {
			b.owner = null;
		});

		// Set
		$.each(balls, function(i, b) {
			var $ball = $('#ball_' + b[1] + '_' + b[2] + '_' + b[3]);
			$ball
				.removeClass(Abalone.oppositeColor(b[0]))
				.addClass(b[0]);
			$ball[0].owner = b[0];
		});
	...
	/**
	 * Handle full status data object from server
	 */
	handleStatus: function(status) {
		// Repopulate map
		this.populateMap(status.balls);

		// Whose turn it is
		this.m_szTurnColor = status.waiting_for_turn ? this.m_szOpponentColor : this.m_szPlayerColor;
		$body.removeClass('go').removeClass('no-go');
		if ( !status.waiting_for_player ) {
			$body.addClass(status.waiting_for_turn ? 'no-go' : 'go');
		}

		// Names
		this.handleNames(status);
	 ...
	 */
	handleNames: function(status) {
		$('#opponent-name').text(status.opponent_name);
		if ( status.name ) {
			$('#player-name').val(status.name);
		}
